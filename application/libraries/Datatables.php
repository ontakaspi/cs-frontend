      ...
      if($sSearch != ''){
        ...
        for($i = 0; $i < count($mColArray); $i++){
          ...
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)){
            if($this->check_cType()){
              //die();
              if($this->select[$mColArray[$i]['data']]!='')
                $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' OR ";
            }else{
              if(isset($this->columns[$i]))
                if($this->select[$this->columns[$i]]!='' && strripos($this->select[$this->columns[$i]], 'CASE WHEN') === FALSE)
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach($this->add_columns as $field => $val){
         if($this->check_cType()){
            ...
            $bbData[$row_key][$field] = $this->exec_replace($val, $aaData[$row_key]);
         }else{
            $var = $val['replacement'][$i];
            //$aaData[$row_key][$var] = $this->exec_replace($val, $aaData[$row_key]);
            $bbData[$row_key][$field] = $this->exec_replace($val, $aaData[$row_key]);
          }
          $i = $i + 1;
        }
        ...
        foreach($this->edit_columns as $modkey => $modval){
          foreach($modval as $val){
            $aaData[$row_key][ ($this->check_cType())? $modval : array_search($modval, $this->columns) ] = $this->exec_replace($val, $aaData[$row_key] );
          }
        }
